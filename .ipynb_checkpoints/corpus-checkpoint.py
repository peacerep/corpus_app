...
def main():
    ...
    # Search box for multiple terms
    search_terms_input = st.text_input("Enter search terms (comma-separated):", "")
    
    if search_terms_input:
        search_term_list = [term.strip() for term in search_terms_input.split(',')]  # Rename to search_term_list
        ...
        if not results_df.empty:
            ...
            # User input for x-axis and y-axis
            x_axis_field = st.selectbox("Select X-axis field:", results_df.columns.tolist())
            y_axis_selection = st.selectbox("Select Y-axis metric:", ["Number of Agreements", "Count of Mentions"])
            
            fig = plot_mentions_distribution(results_df, search_term_list, x_axis_field, y_axis_selection)
            if fig:
                st.plotly_chart(fig)
        else:
